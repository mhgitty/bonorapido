...
export function heroButtonsFor(doc: any): HeroButton[] {
  ...
  if (doc?.showComparisonTable && doc?.comparisonTable && doc?.heroCompareButton) {
    out.push({ text: doc.heroCompareButtonText || 'Ver todos los bonos', targetId: 'comparison-list', variant: 'solid' })
  }

  const types = new Set(((doc?.body as any[]) || []).map((b: any) => b?._type))
  if (types.has('prosConsBlock')) out.push({ text: 'Ventajas y desventajas', targetId: 'pros-cons', variant: 'outline' })
  if (types.has('howToBlock'))    out.push({ text: 'Cómo hacerlo', targetId: 'how-to',    variant: 'outline' })
  if (types.has('faqBlock'))      out.push({ text: 'Preguntas frecuentes', targetId: 'faq',       variant: 'outline' })
...
export function compareButtonList(doc: any): HeroButton[] {
  if (doc?.showComparisonTable && doc?.comparisonTable && doc?.heroCompareButton) {
    return [{ text: doc.heroCompareButtonText || 'Ver todos los bonos', targetId: 'comparison-list', variant: 'solid' }]
  }
  return []
}
