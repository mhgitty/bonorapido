...
export default async function HomePage() {
  ...
  const latestSectionTitle   = hp?.latestSectionTitle   || 'Lo último'
  const casinoReviewsTitle   = hp?.casinoReviewsTitle   || 'Mejores reseñas de casinos'
  const topRatedTitle        = hp?.topRatedTitle        || 'Casinos mejor valorados'
  const featuredSectionTitle = hp?.featuredSectionTitle || 'Destacados'

  const trustItems: { _key: string; icon?: string; title: string; body: string }[] = hp?.trustItems?.length > 0
    ? hp.trustItems
    : [
        { _key: 'trust-1', icon: 'cup-star',           title: 'La confianza de marcas y jugadores', body: 'Operadores, desarrolladores de juegos, jugadores, grandes medios y muchas figuras clave recomiendan Bonorapido como su primera opción.' },
        { _key: 'trust-2', icon: 'medal-ribbons-star',  title: 'Líderes del sector', body: 'La única plataforma de noticias, reseñas y rankings de este tipo en el mundo, con más de una década de experiencia.' },
        { _key: 'trust-3', icon: 'scale',               title: 'Independientes y transparentes', body: 'Propiedad total, sin inversores ni influencias externas. Contamos con periodistas independientes de todo el mundo.' },
      ]
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
                      ...
                      {featuredPost.publishedAt && (
                        <span style={{ fontSize: '12px', color: 'var(--text-muted, #888)' }}>
                          {new Date(featuredPost.publishedAt).toLocaleDateString('es-ES', { day: 'numeric', month: 'short', year: 'numeric' })}
                        </span>
                      )}
                    </div>
                  </a>
            ...
            <div style={{
                    ...
                    {topBookmakers.map((bm: any, i: number) => (
                      <div key={bm._id} style={{
                        ...
                        <div style={{ display: 'flex', gap: '6px', flexShrink: 0 }}>
                          ...
                          {bm.slug?.current && (
                            <a href={`/resenas/${bm.slug.current}/`} style={{
                              display: 'inline-block',
                              padding: '5px 10px',
              ...
              <div style={{ textAlign: 'center', marginTop: '28px' }}>
                <a href="/resenas/" style={{
                  ...
                  textDecoration: 'none',
                  background: 'transparent',
                }}>Ver todas las reseñas de casinos</a>
              </div>
            </div>
          </section>
        )}
                ...
                {featuredCategories.map(cat => (
                  <div key={cat.slug}>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '16px' }}>
                      ...
                      <a href={`/noticias/${cat.slug}/`} style={{
                        fontSize: '13px',
                        color: 'var(--green)',
                        textDecoration: 'none',
                        fontWeight: 500,
