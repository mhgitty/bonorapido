...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!game) return cmsFallbackMetadata('global', ['juegos-de-casino', slug])
  const title = game.metaTitle || `${game.name} — Juega en los mejores casinos online`
  const description = game.metaDescription || `Encuentra los mejores casinos online para jugar a ${game.name}. Compara bonos y regístrate hoy.`
  const canonical = `${BASE}/juegos-de-casino/${slug}/`
...
export default async function CasinoGamePage({ params }: Props) {
  ...
  if (!game) {
    ...
    const fallback = await renderCmsFallback('global', ['juegos-de-casino', slug])
  ...
  const jsonLd = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 2, name: 'Juegos de casino', item: `${BASE}/juegos-de-casino/` },
    ...
    <>
      ...
      <div style={{ background: 'var(--bg-hero)', paddingTop: '32px', paddingBottom: '32px', borderBottom: '1px solid var(--border)' }}>
        <div style={{ maxWidth: '1250px', margin: '0 auto', padding: '0 15px' }}>
          <Breadcrumbs crumbs={[
            { label: 'Inicio', href: '/' },
            { label: 'Juegos de casino', href: '/juegos-de-casino/' },
            { label: game.name },
          ]} />
        ...
        <div className="section">
          <div style={{ maxWidth: '860px', margin: '0 auto' }}>
            <h2 style={{ fontFamily: 'var(--font-display)', fontSize: '20px', fontWeight: 700, marginBottom: '16px', color: 'var(--text)' }}>
              Casinos donde puedes jugar a {game.name}
            </h2>
