...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!provider) return {}
  const title = replaceDateVars(provider.metaTitle || `Casinos online con ${provider.name} — los mejores casinos de ${provider.name}`)
  const description = replaceDateVars(provider.metaDescription || `Encuentra los mejores casinos online con juegos de ${provider.name}. Compara bonos y puntuaciones.`)
  const canonical = `${BASE}/casinos-online/proveedores/${slug}/`
...
export default async function SoftwareSlugPage({ params }: Props) {
  ...
  const jsonLd = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 2, name: 'Proveedores', item: `${BASE}/casinos-online/proveedores/` },
    ...
    <>
      ...
      <div style={{ background: 'var(--bg-hero)', paddingTop: '32px', paddingBottom: '0' }}>
        <div style={{ maxWidth: '1250px', margin: '0 auto', padding: '0 15px' }}>
          <Breadcrumbs crumbs={[
            { label: 'Inicio',     href: '/' },
            { label: 'Proveedores', href: '/casinos-online/proveedores/' },
            { label: provider.name },
          ]} />
        </div>
      </div>
      ...
      {providerSlots.length > 0 && (
        <div className="section">
          <SlotsArchive slots={providerSlots as any} basePath="/tragamonedas" flag="🌍" hideProviders mode="loadmore" loadStep={6} title={`Tragamonedas de ${provider.name}`} />
        </div>
      )}
            ...
            {provider.casinos.map((casino: any) => (
              <div key={casino._id} style={{
                ...
                <div style={{ display: 'flex', gap: '8px', flexShrink: 0 }}>
                  ...
                  <Link href={`/resenas/${casino.slug.current}/`}
