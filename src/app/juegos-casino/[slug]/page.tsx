...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const canonical = `${BASE}/juegos-casino/${slug}/`
...
export default async function CasinoGamePage({ params }: Props) {
  ...
  const jsonLd = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 2, name: 'Juegos de casino', item: `${BASE}/juegos-casino/` },
    ...
    <>
      ...
      <div style={{ background: 'var(--bg-hero)', paddingTop: '32px', paddingBottom: '32px', borderBottom: '1px solid var(--border)' }}>
        <div style={{ maxWidth: '1250px', margin: '0 auto', padding: '0 15px' }}>
          <Breadcrumbs crumbs={[
            { label: 'Inicio', href: '/' },
            { label: 'Juegos de casino', href: '/juegos-casino/' },
            { label: game.name },
          ]} />
