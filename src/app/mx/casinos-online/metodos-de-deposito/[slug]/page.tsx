...
import { getPaymentMethodBySlugMx, client } from '@/lib/sanity'
...
export async function generateStaticParams() {
  const methods = await client.fetch<Array<{ slug: { current: string } }>>(
    `*[_type == "paymentMethod" && market == "mx" && defined(slug.current)] { slug }`
  ).catch(() => [])
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const method = await getPaymentMethodBySlugMx(slug).catch(() => null)
  if (!method) return cmsFallbackMetadata('mx', ['casinos-online', 'metodos-de-deposito', slug])
  const title = replaceDateVars(method.metaTitle || `Casinos con ${method.name} en México — paga con ${method.name}`)
  const description = replaceDateVars(method.metaDescription || `Encuentra los mejores casinos online en México que aceptan ${method.name}. Compara tiempos de retirada, comisiones y bonos.`)
  const canonical = `${BASE}/mx/casinos-online/metodos-de-deposito/${slug}/`
...
export default async function AuPaymentSlugPage({ params }: Props) {
  ...
  if (!method) {
    ...
    const fallback = await renderCmsFallback('mx', ['casinos-online', 'metodos-de-deposito', slug])
  ...
  const jsonLd = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: 'Métodos de pago', item: `${BASE}/mx/casinos-online/metodos-de-deposito/` },
      ...
      <div style={{ background: 'var(--bg-hero)', paddingTop: '32px', paddingBottom: '0' }}>
        <div style={{ maxWidth: '1250px', margin: '0 auto', padding: '0 15px' }}>
          <Breadcrumbs crumbs={[
            ...
            { label: 'Métodos de pago', href: '/mx/casinos-online/metodos-de-deposito/' },
