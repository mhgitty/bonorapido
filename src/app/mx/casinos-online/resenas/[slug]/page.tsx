...
import { getBookmakerBySlugMx, getPosts, getSiteSettings, clientNoCdn, getHreflangScript } from '@/lib/sanity'
...
export async function generateStaticParams() {
  const bookmakers = await clientNoCdn.fetch<Array<{ slug: { current: string } }>>(
    `*[_type == "bookmaker" && market == "mx" && defined(slug.current)] { slug }`
  ).catch(() => [])
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const bm = await getBookmakerBySlugMx(slug).catch(() => null)
  if (!bm) return {}
  const title = replaceDateVars(bm.metaTitle || `Reseña de ${bm.name} en México — bonos y ofertas`)
  const description = replaceDateVars(bm.metaDescription || `Lee nuestra reseña de ${bm.name} para México. Consulta el bono, los requisitos de apuesta y nuestra puntuación.`)
  const canonical = `${BASE}/mx/casinos-online/resenas/${slug}/`
...
export default async function CaReviewSlugPage({ params }: Props) {
  ...
  const [bm, latestPosts, settings] = await Promise.all([
    getBookmakerBySlugMx(slug).catch(() => null),
    getPosts(6),
    ...
    '@graph': [
      {
        ...
        itemListElement: [
          ...
          { '@type': 'ListItem', position: 3, name: 'Reseñas de casinos', item: `${BASE}/mx/casinos-online/resenas/` },
      ...
      <div style={{ background: 'var(--bg-hero)', borderBottom: '1px solid var(--border)', padding: '40px 15px 32px' }}>
        <div style={{ maxWidth: '1250px', margin: '0 auto' }}>
          <Breadcrumbs crumbs={[
            ...
            { label: 'Casino online',  href: '/mx/casinos-online/' },
            { label: 'Reseñas de casinos', href: '/mx/casinos-online/resenas/' },
                ...
                {bm.minIndbetaling != null && (
                  <div style={{ background: 'var(--bg-raised)', border: '1px solid var(--border)', borderRadius: '10px', padding: '10px 14px', display: 'flex', alignItems: 'center', gap: '10px' }}>
                    ...
                    <div>
                      ...
                      <div style={{ fontSize: '14px', fontWeight: 600, color: 'var(--text)' }}>MXN {bm.minIndbetaling}</div>
                    </div>
                  </div>
                )}
                {bm.gennemspilskrav && (
