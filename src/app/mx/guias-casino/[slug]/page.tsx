...
import { getCasinoGuideBySlugMx, getSiteSettings, client } from '@/lib/sanity'
...
export async function generateStaticParams() {
  const guides = await client.fetch<Array<{ slug: { current: string } }>>(
    `*[_type == "casinoGuide" && market == "mx" && defined(slug.current)] { slug }`
  ).catch(() => [])
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const guide = await getCasinoGuideBySlugMx(slug).catch(() => null)
  if (!guide) return {}
  const canonical = `${BASE}/mx/guias-casino/${slug}/`
...
export default async function AuCasinoGuidePage({ params }: Props) {
  const { slug } = await params
  const [guide, settings] = await Promise.all([
    getCasinoGuideBySlugMx(slug).catch(() => null),
  ...
  const breadcrumbs = [
    ...
    { label: 'Guías de casino', href: '/mx/guias-casino/' },
    { label: guide.title },
  ]
  ...
  const jsonLd = {
    ...
    '@graph': [
      ...
      { '@type': 'WebPage', '@id': `${canonical}#webpage`, url: canonical, name: replaceDateVars(guide.title), inLanguage: 'es-MX', publisher: { '@type': 'Organization', name: 'Bonorapido', url: BASE } },
    ],
  }
