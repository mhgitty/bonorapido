...
import { getBonusBySlugMx, getPageByPathMx, getSiteSettings, getHreflangScript, client } from '@/lib/sanity'
...
export async function generateStaticParams() {
  const bonuses = await client.fetch<Array<{ slug: { current: string } }>>(
    `*[_type == "bonus" && market == "mx" && active == true && defined(slug.current)] { slug }`
  ).catch(() => [])
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const canonical = `${BASE}/mx/casino-online/bonos/${slug}/`

  // CMS page takes priority
  const page = await getPageByPathMx(['casino-online', 'bonos', slug]).catch(() => null)
  if (page) {
  ...
  const bonus = await getBonusBySlugMx(slug).catch(() => null)
  if (!bonus) return {}
...
export default async function AuBonusSlugPage({ params }: Props) {
  ...
  const [page, bonus, settings] = await Promise.all([
    getPageByPathMx(['casino-online', 'bonos', slug]).catch(() => null),
    getBonusBySlugMx(slug).catch(() => null),
  ...
  if (page) {
    ...
    const breadcrumbs = [
      ...
      { label: 'Bonos', href: '/mx/casino-online/bonos/' },
    ...
    const jsonLd = {
      ...
      '@graph': [
        ...
        { '@type': 'WebPage', '@id': `${canonical}#webpage`, url: canonical, name: replaceDateVars(page.title), inLanguage: 'es-MX', publisher: { '@type': 'Organization', name: 'Bonorapido', url: BASE } },
      ],
    }
    ...
    '@graph': [
      {
        ...
        itemListElement: [
          ...
          { '@type': 'ListItem', position: 3, name: 'Bonos',         item: `${BASE}/mx/casino-online/bonos/` },
          { '@type': 'ListItem', position: 4, name: slug.replace(/-/g, ' ').replace(/^\w/, c => c.toUpperCase()), item: canonical },
        ],
      },
      { '@type': 'WebPage', '@id': `${canonical}#webpage`, url: canonical, name: bonus.title, inLanguage: 'es-MX', publisher: { '@type': 'Organization', name: 'Bonorapido', url: BASE } },
    ],
  }
  ...
  return (
    <>
      ...
      <BonusHero
        bonusListHref="/mx/casino-online/bonos/"
        title={bonus.title}
