...
import { getPageByPathMx, getSiteSettings, getHreflangScript, client } from '@/lib/sanity'
...
export async function generateStaticParams() {
  const pages = await client.fetch<Array<{
    ...
    `*[_type == "page" && market == "mx" && defined(slug.current)] {
      slug,
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const canonical = `${BASE}/mx/${slug.join('/')}/`
...
export default async function CaSlugPage({ params }: Props) {
  ...
  const breadcrumbs: { label: string; href?: string }[] = [
    ...
    ...slug.slice(0, -1).map((seg, idx) => ({
      label: slugLabel(seg),
      href: `/mx/${slug.slice(0, idx + 1).join('/')}/`,
    })),
