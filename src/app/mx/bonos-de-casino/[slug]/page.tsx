...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const canonical = `${BASE}/mx/bonos-de-casino/${slug}/`

  // CMS page takes priority
  const page = await getPageByPathMx(['bonos-de-casino', slug]).catch(() => null)
  if (page) {
...
export default async function AuBonusSlugPage({ params }: Props) {
  ...
  const [page, bonus, settings] = await Promise.all([
    getPageByPathMx(['bonos-de-casino', slug]).catch(() => null),
  ...
  if (page) {
    ...
    const breadcrumbs = [
      ...
      { label: 'Bonos', href: '/mx/bonos-de-casino/' },
    ...
    '@graph': [
      {
        ...
        itemListElement: [
          ...
          { '@type': 'ListItem', position: 3, name: 'Bonos',         item: `${BASE}/mx/bonos-de-casino/` },
  ...
  return (
    <>
      ...
      <BonusHero
        bonusListHref="/mx/bonos-de-casino/"
        title={bonus.title}
