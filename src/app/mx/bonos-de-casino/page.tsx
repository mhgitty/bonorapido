...
import { getPageByPathMx, getSiteSettings, getHreflangScript } from '@/lib/sanity'
...
const CANONICAL = `${BASE}/mx/bonos-de-casino/`

export async function generateMetadata(): Promise<Metadata> {
  const page = await getPageByPathMx(['bonos-de-casino']).catch(() => null)
  if (!page) return {}
...
export default async function AuBonusPage() {
  const [page, settings] = await Promise.all([
    getPageByPathMx(['bonos-de-casino']).catch(() => null),
