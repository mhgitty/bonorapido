...
import { getCasinoGameBySlugMx, client } from '@/lib/sanity'
...
export async function generateStaticParams() {
  const games = await client.fetch<Array<{ slug: { current: string } }>>(
    `*[_type == "casinoGame" && market == "mx" && defined(slug.current)] { slug }`
  ).catch(() => [])
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const game = await getCasinoGameBySlugMx(slug).catch(() => null)
  if (!game) return cmsFallbackMetadata('mx', ['juegos-de-casino', slug])
  const title = game.metaTitle || `${game.name} — Mejores casinos online en México`
  const description = game.metaDescription || `Encuentra los mejores casinos online en México para jugar a ${game.name}. Compara bonos y regístrate hoy.`
  const canonical = `${BASE}/mx/juegos-de-casino/${slug}/`
...
export default async function AuCasinoGamePage({ params }: Props) {
  ...
  if (!game) {
    ...
    const fallback = await renderCmsFallback('mx', ['juegos-de-casino', slug])
  ...
  const jsonLd = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 2, name: 'Juegos de casino', item: `${BASE}/mx/juegos-de-casino/` },
      ...
      <div style={{ background: 'var(--bg-hero)', paddingTop: '32px', paddingBottom: '32px', borderBottom: '1px solid var(--border)' }}>
        <div style={{ maxWidth: '1250px', margin: '0 auto', padding: '0 15px' }}>
          <Breadcrumbs crumbs={[
            ...
            { label: 'Juegos de casino', href: '/mx/juegos-de-casino/' },
        ...
        <div className="section">
          <div style={{ maxWidth: '860px', margin: '0 auto' }}>
            <h2 style={{ fontFamily: 'var(--font-display)', fontSize: '20px', fontWeight: 700, marginBottom: '16px', color: 'var(--text)' }}>
              Casinos en México donde puedes jugar a {game.name}
            </h2>
