...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const description = replaceDateVars(page.metaDescription || `Consulta la clasificación de ${page.leagueName}, actualizada en directo.`)
  const canonical = `${BASE}/futbol/clasificaciones/${slug}/`
    ...
    '@graph': [
      {
        ...
        itemListElement: [
          ...
          { '@type': 'ListItem', position: 2, name: 'Fútbol', item: `${BASE}/futbol/` },
          { '@type': 'ListItem', position: 3, name: 'Clasificaciones', item: `${BASE}/futbol/clasificaciones/` },
      ...
      <section className="hero-section" style={{ background: 'var(--bg-hero)', borderBottom: '1px solid var(--border)' }}>
        <div style={{ maxWidth: '1250px', margin: '0 auto' }}>
          <Breadcrumbs crumbs={[
            ...
            { label: 'Clasificaciones', href: '/futbol/clasificaciones' },
  ...
  if (!rows.length) {
    return (
      <div style={{
        ...
        No hay datos de clasificación disponibles para {leagueName}.
      </div>
    )
  }
  ...
  return (
    <div style={{
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13.5px' }}>
          <thead>
            <tr style={{ borderBottom: '1px solid var(--border)', background: 'var(--bg-raised)' }}>
              ...
              <Th title="Puntos" style={{ color: 'var(--green)', fontWeight: 700 }}>Pts</Th>
            </tr>
          </thead>
          <tbody>
            {rows.map((row, i) => (
      ...
      <div style={{ padding: '10px 20px', borderTop: '1px solid var(--border-faint)', fontSize: '11px', color: 'var(--text-faint)' }}>
        Datos de Sportsmonks · Se actualiza cada 60 min
      </div>
    </div>
  )
}
