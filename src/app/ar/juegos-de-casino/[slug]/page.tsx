...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!game) return cmsFallbackMetadata('ar', ['juegos-de-casino', slug])
  ...
  const canonical = `${BASE}/ar/juegos-de-casino/${slug}/`
...
export default async function CaCasinoGamePage({ params }: Props) {
  ...
  if (!game) {
    ...
    const fallback = await renderCmsFallback('ar', ['juegos-de-casino', slug])
  ...
  const jsonLd = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 2, name: 'Juegos de casino', item: `${BASE}/ar/juegos-de-casino/` },
      ...
      <div style={{ background: 'var(--bg-hero)', paddingTop: '32px', paddingBottom: '32px', borderBottom: '1px solid var(--border)' }}>
        <div style={{ maxWidth: '1250px', margin: '0 auto', padding: '0 15px' }}>
          <Breadcrumbs crumbs={[
            ...
            { label: 'Juegos de casino', href: '/ar/juegos-de-casino/' },
