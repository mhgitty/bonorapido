...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const canonical = `${BASE}/ar/casino-online/bonos/${slug}/`

  // CMS page takes priority
  const page = await getPageByPathAr(['casino-online', 'bonos', slug]).catch(() => null)
  if (page) {
...
export default async function CaBonusSlugPage({ params }: Props) {
  ...
  const [page, bonus, settings] = await Promise.all([
    getPageByPathAr(['casino-online', 'bonos', slug]).catch(() => null),
  ...
  if (page) {
    ...
    const breadcrumbs = [
      ...
      { label: 'Bonos', href: '/ar/casino-online/bonos/' },
    ...
    '@graph': [
      {
        ...
        itemListElement: [
          ...
          { '@type': 'ListItem', position: 3, name: 'Bonos',         item: `${BASE}/ar/casino-online/bonos/` },
  ...
  return (
    <>
      ...
      <BonusHero
        bonusListHref="/ar/casino-online/bonos/"
        title={bonus.title}
