...
import { getSlotmachinesForArchive, getArchiveFeaturedSlotIds, getPageByPathAr, getSiteSettings, getHreflangScript } from '@/lib/sanity'
import type { Metadata } from 'next'

export const revalidate = 3600
const BASE = 'https://bonorapido.com'
const MARKET = 'ar' as const
...
export async function generateMetadata(): Promise<Metadata> {
  const page = await getPageByPathAr(['tragamonedas']).catch(() => null)
...
export default async function SlotsArchivePage() {
  const [slots, featuredIds, page] = await Promise.all([
    ...
    getPageByPathAr(['tragamonedas']).catch(() => null),
  ])
