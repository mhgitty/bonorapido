...
import { getPageBySlugAr, getCasinoGuides, getSiteSettings } from '@/lib/sanity'
...
const CANONICAL = `${BASE}/ar/guias-casino/`

export async function generateMetadata(): Promise<Metadata> {
  const page = await getPageBySlugAr('guias-casino').catch(() => null)
  const title = replaceDateVars(page?.metaTitle || page?.title || 'Guías de casino')
  const description = replaceDateVars(page?.metaDescription || page?.intro || 'Guías de casino y tutoriales para jugadores argentinos.')
...
export default async function CaCasinoGuidesPage() {
  const [page, guides, settings] = await Promise.all([
    getPageBySlugAr('guias-casino').catch(() => null),
    getCasinoGuides('ar').catch(() => []),
    getSiteSettings().catch(() => null),
  ])
  const author = (page as any)?.author ?? settings?.defaultAuthor ?? null
  const title = page?.title || 'Guías de casino'
  const intro = page?.intro || 'Explora nuestras guías de casino para jugadores argentinos.'
  ...
  return (
    <>
      ...
      <HeroSection
        ...
        breadcrumbs={[{ label: 'Inicio', href: '/ar/' }, { label: 'Guías de casino' }]}
      />
      ...
      <GuidesArchive guides={guides as any[]} hrefPrefix="/ar/guias-casino" />
