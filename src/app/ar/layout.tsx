...
function resolveUrl(item: {
  ...
  if (item.pageSlug) {
    const prefix = item.pageMarket === 'ar' ? '/ar' : item.pageMarket === 'mx' ? '/mx' : ''
  ...
  if (item.bookmakerSlug) return `/ar/casinos-online/resenas/${item.bookmakerSlug}/`
  if (item.softwareSlug) return `/ar/casinos-online/proveedores/${item.softwareSlug}/`
  if (item.paymentMethodSlug) return `/ar/casinos-online/metodos-de-deposito/${item.paymentMethodSlug}/`
...
export default async function CaLayout({ children }: { children: React.ReactNode }) {
  const [ms, gs] = await Promise.all([
    getMarketSettings('ar').catch(() => null),
  ...
  const tagline    = ms?.footerTagline    || gs?.footerTagline    || 'Tu guía independiente de casinos online para jugadores argentinos.'
  ...
  return (
    <>
      <Navbar navItems={navItems} logoHref="/ar/" />
      {children}
      <Footer tagline={tagline} columns={columns} note={note} disclaimer={disclaimer} socialLinks={ms?.socialLinks ?? gs?.socialLinks} market="ar" />
    </>
  )
}
