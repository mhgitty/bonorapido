...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!method) return cmsFallbackMetadata('ar', ['casinos-online', 'metodos-de-deposito', slug])
  ...
  const canonical = `${BASE}/ar/casinos-online/metodos-de-deposito/${slug}/`
...
export default async function CaPaymentSlugPage({ params }: Props) {
  ...
  if (!method) {
    ...
    const fallback = await renderCmsFallback('ar', ['casinos-online', 'metodos-de-deposito', slug])
  ...
  const jsonLd = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: 'Métodos de pago', item: `${BASE}/ar/casinos-online/metodos-de-deposito/` },
      ...
      <div style={{ background: 'var(--bg-hero)', paddingTop: '32px', paddingBottom: '0' }}>
        <div style={{ maxWidth: '1250px', margin: '0 auto', padding: '0 15px' }}>
          <Breadcrumbs crumbs={[
            ...
            { label: 'Métodos de pago', href: '/ar/casinos-online/metodos-de-deposito/' },
