...
import { getSoftwareBySlugAr, getSlotsByProvider, client } from '@/lib/sanity'
...
export async function generateStaticParams() {
  const providers = await client.fetch<Array<{ slug: { current: string } }>>(
    `*[_type == "software" && market == "ar" && defined(slug.current)] { slug }`
  ).catch(() => [])
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const provider = await getSoftwareBySlugAr(slug).catch(() => null)
  if (!provider) return {}
  const title = replaceDateVars(provider.metaTitle || `Casinos ${provider.name} en Argentina — mejores casinos online con ${provider.name}`)
  const description = replaceDateVars(provider.metaDescription || `Encuentra los mejores casinos online en Argentina con juegos de ${provider.name}. Compara bonos y puntuaciones.`)
  const canonical = `${BASE}/ar/casinos-online/proveedores/${slug}/`
...
export default async function CaSoftwareSlugPage({ params }: Props) {
  ...
  const jsonLd = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: 'Proveedores',      item: `${BASE}/ar/casinos-online/proveedores/` },
    ...
    <>
      ...
      <div style={{ background: 'var(--bg-hero)', paddingTop: '32px', paddingBottom: '0' }}>
        <div style={{ maxWidth: '1250px', margin: '0 auto', padding: '0 15px' }}>
          <Breadcrumbs crumbs={[
            ...
            { label: 'Proveedores',      href: '/ar/casinos-online/proveedores/' },
      ...
      {providerSlots.length > 0 && (
        <div className="section">
          <SlotsArchive slots={providerSlots as any} basePath="/ar/tragamonedas" flag="🇦🇷" hideProviders mode="loadmore" loadStep={6} title={`Tragamonedas de ${provider.name}`} />
        </div>
      )}
      ...
      {provider.casinos && provider.casinos.length > 0 && (
        <div className="section">
          <h2 style={{ fontFamily: 'var(--font-display)', fontSize: '20px', fontWeight: 700, marginBottom: '16px', color: 'var(--text)' }}>
            Casinos en Argentina con {provider.name}
          </h2>
            ...
            {provider.casinos.map((casino: any) => (
              <div key={casino._id} style={{
                ...
                <div style={{ display: 'flex', gap: '8px', flexShrink: 0 }}>
                  ...
                  <Link href={`/ar/casinos-online/resenas/${casino.slug.current}/`}
