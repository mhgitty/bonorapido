...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!author) return {}
  const title = author.metaTitle || `${author.name} — ${author.role || 'Autor'} en Bonorapido`
  const description = author.metaDescription || author.intro || author.bio || `Artículos y reseñas de ${author.name}.`
  return {
    title,
    description,
    alternates: { canonical: `${BASE}/autor/${slug}/` },
    openGraph: { title, description, url: `${BASE}/autor/${slug}/` },
  }
}
    ...
    '@graph': [
      ...
      },
      {
        ...
        itemListElement: [
          ...
          { '@type': 'ListItem', position: 2, name: author.name, item: `${BASE}/autor/${slug}/` },
        ],
      },
    ],
  }
  ...
  return (
    <>
      ...
      <div style={{ background: 'var(--bg-footer)', padding: '48px 24px 56px' }}>
        <div style={{ maxWidth: '1250px', margin: '0 auto' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '36px', fontSize: '13px' }}>
            ...
            <span style={{ color: 'rgba(255,255,255,0.55)' }}>Autores</span>
            <span style={{ color: 'rgba(255,255,255,0.25)' }}>›</span>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '20px' }}>
              ...
              <div style={{ display: 'flex', background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '10px', overflow: 'hidden' }}>
                <div style={{ padding: '12px 20px', textAlign: 'center', borderRight: '1px solid rgba(255,255,255,0.1)' }}>
                  <div style={{ fontSize: '11px', color: 'rgba(255,255,255,0.4)', textTransform: 'uppercase', letterSpacing: '0.5px', marginBottom: '2px' }}>Artículos</div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              ...
              {author.education && (
                <div style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '10px', padding: '16px 20px' }}>
                  <div style={{ fontSize: '13px', fontWeight: 700, color: '#fff', marginBottom: '4px' }}>Formación</div>
              ...
              {author.expertise && (
                <div style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '10px', padding: '16px 20px' }}>
                  <div style={{ fontSize: '13px', fontWeight: 700, color: '#fff', marginBottom: '4px' }}>Especialidad</div>
              ...
              {(author.intro || author.bio) && (
                <div style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '10px', padding: '16px 20px' }}>
                  <div style={{ fontSize: '13px', fontWeight: 700, color: '#fff', marginBottom: '8px' }}>Resumen</div>
      ...
      <div className="section">
        ...
        {posts.length === 0 ? (
          <p style={{ color: 'var(--text-muted)', fontSize: '15px' }}>Todavía no hay artículos.</p>
        ) : (
