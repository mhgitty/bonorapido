...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!bm) return {}
  const title = replaceDateVars(bm.metaTitle || `Reseña de ${bm.name} — bono y ofertas`)
  const description = replaceDateVars(bm.metaDescription || `Lee nuestra reseña de ${bm.name}. Consulta el bono, los requisitos de apuesta y nuestra puntuación.`)
  const canonical = `${BASE}/resenas/${slug}/`
    ...
    '@graph': [
      {
        ...
        itemListElement: [
          ...
          { '@type': 'ListItem', position: 2, name: 'Reseñas de casinos', item: `${BASE}/resenas/` },
    ...
    <>
      ...
      <div style={{ background: 'var(--bg-hero)', borderBottom: '1px solid var(--border)', padding: '40px 15px 32px' }}>
        <div style={{ maxWidth: '1250px', margin: '0 auto' }}>
          <Breadcrumbs crumbs={[
            { label: 'Inicio', href: '/' },
            { label: 'Reseñas de casinos', href: '/resenas/' },
            { label: bm.name },
          ]} />
              ...
              <div className="bm-hero-stats">
                {bm.minIndbetaling != null && (
                  <div style={{ background: 'var(--bg-raised)', border: '1px solid var(--border)', borderRadius: '10px', padding: '10px 14px', display: 'flex', alignItems: 'center', gap: '10px' }}>
                    ...
                    <div>
                      <div style={{ fontSize: '10px', color: 'var(--text-faint)', textTransform: 'uppercase', letterSpacing: '0.4px', marginBottom: '1px' }}>Depósito mínimo</div>
                      <div style={{ fontSize: '14px', fontWeight: 600, color: 'var(--text)' }}>{bm.minIndbetaling} €</div>
                    </div>
                  </div>
                )}
                {bm.gennemspilskrav && (
                  <div style={{ background: 'var(--bg-raised)', border: '1px solid var(--border)', borderRadius: '10px', padding: '10px 14px', display: 'flex', alignItems: 'center', gap: '10px' }}>
                    ...
                    <div>
                      <div style={{ fontSize: '10px', color: 'var(--text-faint)', textTransform: 'uppercase', letterSpacing: '0.4px', marginBottom: '1px' }}>Requisitos de apuesta</div>
                ...
                {bm.lanceringsdato && (
                  <div style={{ background: 'var(--bg-raised)', border: '1px solid var(--border)', borderRadius: '10px', padding: '10px 14px', display: 'flex', alignItems: 'center', gap: '10px' }}>
                    ...
                    <div>
                      <div style={{ fontSize: '10px', color: 'var(--text-faint)', textTransform: 'uppercase', letterSpacing: '0.4px', marginBottom: '1px' }}>Fundado en</div>
