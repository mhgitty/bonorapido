...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const canonical = `${BASE}/casino-online/proveedores/${slug}/`
...
export default async function SoftwareSlugPage({ params }: Props) {
  ...
  const jsonLd = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 2, name: 'Proveedores', item: `${BASE}/casino-online/proveedores/` },
    ...
    <>
      ...
      <div style={{ background: 'var(--bg-hero)', paddingTop: '32px', paddingBottom: '0' }}>
        <div style={{ maxWidth: '1250px', margin: '0 auto', padding: '0 15px' }}>
          <Breadcrumbs crumbs={[
            { label: 'Inicio',     href: '/' },
            { label: 'Proveedores', href: '/casino-online/proveedores/' },
            { label: provider.name },
          ]} />
        </div>
      </div>
