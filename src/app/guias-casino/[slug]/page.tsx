...
export async function generateStaticParams() {
  const guides = await client.fetch<Array<{ slug: { current: string } }>>(
    `*[_type == "casinoGuide" && market == "global" && defined(slug.current)] { slug }`
  ).catch(() => [])
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!guide) return {}
  const canonical = `${BASE}/guias-casino/${slug}/`
...
export default async function CasinoGuidePage({ params }: Props) {
  ...
  const breadcrumbs = [
    { label: 'Inicio', href: '/' },
    { label: 'Guías de casino', href: '/guias-casino/' },
    { label: guide.title },
  ]
  ...
  const jsonLd = {
    ...
    '@graph': [
      ...
      { '@type': 'WebPage', '@id': `${canonical}#webpage`, url: canonical, name: replaceDateVars(guide.title), inLanguage: 'es-ES', publisher: { '@type': 'Organization', name: 'Bonorapido', url: BASE } },
    ],
  }
