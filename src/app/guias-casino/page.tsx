...
const CANONICAL = `${BASE}/guias-casino/`

export async function generateMetadata(): Promise<Metadata> {
  const page = await getPageBySlug('guias-casino').catch(() => null)
  const title = replaceDateVars(page?.metaTitle || page?.title || 'Guías de casino')
  const description = replaceDateVars(page?.metaDescription || page?.intro || 'Guías de casino, estrategias y tutoriales.')
...
export default async function CasinoGuidesPage() {
  const [page, guides, settings] = await Promise.all([
    getPageBySlug('guias-casino').catch(() => null),
  ...
  const intro = page?.intro || 'Explora nuestras guías de casino, estrategias y tutoriales.'
  ...
  return (
    <>
      ...
      <HeroSection
        ...
        breadcrumbs={[{ label: 'Inicio', href: '/' }, { label: 'Guías de casino' }]}
      />
      ...
      <GuidesArchive guides={guides as any[]} hrefPrefix="/guias-casino" />
