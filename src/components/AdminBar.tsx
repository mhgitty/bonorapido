...
function resolveEditUrl(pathname: string): string | null {
  ...
  if (p === '/ar')  return '/studio/intent/edit/id=ar-homepage/'
  if (p === '/mx')  return '/studio/intent/edit/id=mx-homepage/'

  // Blog post  /[slug]
  const postMatch = p.match(/^\/([^/]+)$/)
  if (postMatch) return `${s}/type=post,slug.current=="${postMatch[1]}"/`

  // Global review  /resenas/[slug]
...
function pageLabel(pathname: string): string {
  ...
  if (p === '/ar') return 'AR Homepage'
  if (p === '/mx') return 'MX Homepage'
  if (p.match(/\/resenas\//)) return 'Casino review'
  if (p.match(/\/noticias\//))   return 'News'
