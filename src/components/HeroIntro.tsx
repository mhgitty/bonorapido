...
export function HeroIntro({ value }: { value: any[] }) {
  ...
  if (open) {
    return (
      <div>
        ...
        <button type="button" className="hero-readmore" onClick={() => setOpen(false)}>Mostrar menos</button>
      </div>
    )
  }
  ...
  return (
    <p style={{ margin: 0 }}>
      ...
      <button type="button" className="hero-readmore" onClick={() => setOpen(true)}>Leer más</button>
    </p>
  )
}
