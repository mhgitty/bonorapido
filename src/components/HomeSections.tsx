...
import {
  getBookmakersAr, getBookmarkersMx,
  getPaymentMethodsAr, getPaymentMethodsMx,
  getSoftwareProvidersAr, getSoftwareProvidersMx,
  getCasinoGuides,
} from '@/lib/sanity'
...
function CasinoListSection({ bookmakers, section, reviewBase, listBase }: {
  ...
  const title = section.title || 'Casinos mejor valorados'
      ...
      {bookmakers.length > count && (
        <div style={{ textAlign: 'center', marginTop: '20px' }}>
          <Link href={listBase}
            ...
            Ver los {bookmakers.length} casinos →
          </Link>
        </div>
      )}
    </div>
...
export async function HomeSections({ sections, market }: { sections: AnySection[]; market: 'ar' | 'mx' }) {
  ...
  const [bookmakers, payments, software, reviewCasinos, guides] = await Promise.all([
    needsBookmakers
      ? (market === 'ar' ? getBookmakersAr() : getBookmarkersMx()).catch(() => [])
      : Promise.resolve([]),
    needsPayments
      ? (market === 'ar' ? getPaymentMethodsAr() : getPaymentMethodsMx()).catch(() => [])
      : Promise.resolve([]),
    needsSoftware
      ? (market === 'ar' ? getSoftwareProvidersAr() : getSoftwareProvidersMx()).catch(() => [])
      : Promise.resolve([]),
    needsReviews
      ? (market === 'ar' ? getBookmakersAr() : getBookmarkersMx()).catch(() => [])
      : Promise.resolve([]),
          ...
          case 'sectionReviewsArchive': {
            ...
            return (
              <CasinoReviewsArchive
                ...
                title={sec.title || 'Reseñas de casinos'}
          ...
          case 'sectionGuidesArchive': {
            ...
            return (
              <GuidesArchive
                ...
                title={sec.title || 'Guías de casino'}
                intro={sec.intro ? <RichIntro value={sec.intro} /> : undefined}
          ...
          case 'sectionPaymentMethods':
            return (payments as any[]).length > 0 ? (
              <ProviderCardsSection
                ...
                hrefPrefix={payBase}
                seeAllLabel="Ver todos los métodos de pago"
                seeAllHref={`${payBase}/`}
                casinoLabel="casinos"
              />
            ) : null
          ...
          case 'sectionSoftware':
            return (software as any[]).length > 0 ? (
              <ProviderCardsSection
                ...
                hrefPrefix={softBase}
                seeAllLabel="Ver todos los proveedores de software"
                seeAllHref={`${softBase}/`}
                casinoLabel="casinos"
              />
            ) : null
