...
export function CasinoReviewsArchive({ casinos, hrefPrefix = '/resenas', title = 'Todas las reseñas de casinos', intro, seeAllHref, seeAllLabel = 'Ver todas las reseñas', initialCount }: Props) {
    ...
    <div className="section">
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '16px', flexWrap: 'wrap', marginBottom: '18px' }}>
        ...
        <div style={{ position: 'relative', flex: '1 1 240px', maxWidth: '340px' }}>
          ...
          <input
            ...
            placeholder="Buscar casinos…"
            aria-label="Buscar reseñas de casinos"
            style={{
      ...
      {filtered.length === 0 ? (
        <div style={{ textAlign: 'center', padding: '48px', color: 'var(--text-faint)', fontSize: '14px' }}>
          Ningún casino coincide con &ldquo;{query}&rdquo;.
        </div>
      ) : (
                  ...
                  <div style={{ marginTop: 'auto', display: 'flex', flexDirection: 'column', gap: '8px' }}>
                    {c.url && (
                      <a href={c.url} target="_blank" rel="nofollow noopener noreferrer sponsored" style={{
                        ...
                        Visitar casino <Icon name="arrow-right" size={15} color="#fff" />
                      </a>
                    )}
                    <Link href={href} style={{
                      display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '5px',
