...
interface Props {
  guides?: Guide[]
  /** Prefix for the guide link, e.g. "/ar/guias-casino". */
  hrefPrefix?: string
  title?: string
...
export function GuidesArchive({ guides, hrefPrefix = '/guias-casino', title = 'Todas las guías de casino', intro, seeAllHref, seeAllLabel = 'Ver todas las guías de casino', initialCount }: Props) {
    ...
    <div className="section">
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '16px', flexWrap: 'wrap', marginBottom: '18px' }}>
        ...
        <div style={{ position: 'relative', flex: '1 1 240px', maxWidth: '340px' }}>
          ...
          <input
            ...
            aria-label="Buscar guías de casino"
            style={{
      ...
      {filtered.length === 0 ? (
        <div style={{ textAlign: 'center', padding: '48px', color: 'var(--text-faint)', fontSize: '14px' }}>
          Ninguna guía coincide con &ldquo;{query}&rdquo;.
        </div>
      ) : (
              ...
              <Link key={g._id} href={href} style={{
                ...
                <div style={{ padding: '16px 18px', flex: 1, display: 'flex', flexDirection: 'column' }}>
                  ...
                  <span style={{ marginTop: 'auto', display: 'inline-flex', alignItems: 'center', gap: '5px', fontSize: '13.5px', fontWeight: 600, color: 'var(--green)' }}>
                    Leer guía <Icon name="arrow-right" size={14} color="var(--green)" />
                  </span>
                </div>
              </Link>
            )
